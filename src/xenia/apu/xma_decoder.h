...
class XmaDecoder {
 ...
 protected:
  ...
  static constexpr size_t kWorkerPoolSize = kKickGroupCount / kKickGroupsPerWorker;
  struct PoolWorker {
    std::atomic<uint32_t> wake_signal{0};
    std::atomic<uint64_t> pending_mask{0};
  };
  std::array<PoolWorker, kWorkerPoolSize> worker_pool_;
  std::array<kernel::object_ref<kernel::XHostThread>, kWorkerPoolSize> pool_threads_;
